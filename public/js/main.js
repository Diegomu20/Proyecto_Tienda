...
const item = {
  ...
  function initCartPage() {
    ...
    function renderCart() {
      ...
      // Actualizar solo los precios, SIN tocar el selector de medio de pago
      const resumenEl = document.getElementById('resumen-precios');
      if (resumenEl) {
        resumenEl.innerHTML =  `<h3 style="margin-bottom:.75rem">Resumen</h3>
  ...
  try {
    ...
    const medioPago = document.querySelector('input[name="medioPago"]:checked')?.value || 'Efectivo';
    window.location.href = `/confirmacion?orderId=${data.orderId}&medioPago=${encodeURIComponent(medioPago)}`;
  } catch (error) {
